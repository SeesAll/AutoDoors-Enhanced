...
namespace Oxide.Plugins
{
    ...
    public class AutoDoors : RustPlugin
    {
        ...
        private readonly Dictionary<string, string> supportedDoors = new Dictionary<string, string>(StringComparer.Ordinal);
        private readonly Dictionary<ulong, ulong> controllerDoorIdsByManipulator = new Dictionary<ulong, ulong>();
        private readonly Dictionary<ulong, int> controllerDoorCounts = new Dictionary<ulong, int>();

        private static readonly Dictionary<string, string> KnownDoorFallbacks = new Dictionary<string, string>(StringComparer.Ordinal)
        {
        ...
        {
            if (doorManipulator == null || doorManipulator.net == null || doorManipulator.OwnerID == 0) return;

            var manipulatorId = doorManipulator.net.ID.Value;
            var targetDoorId = GetDoorId(doorManipulator.targetDoor);

            controllerDoorIdsByManipulator[manipulatorId] = targetDoorId;
            AddControllerDoorReference(targetDoorId);
        }

        private void OnEntityKill(DoorManipulator doorManipulator)
        {
            if (doorManipulator == null || doorManipulator.net == null) return;

            var manipulatorId = doorManipulator.net.ID.Value;
            ulong targetDoorId;
            if (!controllerDoorIdsByManipulator.TryGetValue(manipulatorId, out targetDoorId)) return;

            controllerDoorIdsByManipulator.Remove(manipulatorId);
            RemoveControllerDoorReference(targetDoorId);
        }
        ...
        {
            if (door == null || door.net == null || !door.IsOpen() || player == null) return;
        ...
        {
            return door != null && door.net != null && controllerDoorCounts.ContainsKey(door.net.ID.Value);
        }

        private static ulong GetDoorId(Door door)
        {
            return door?.net?.ID.Value ?? 0u;
        }

        private void AddControllerDoorReference(ulong doorId)
        {
            if (doorId == 0) return;

            int count;
            controllerDoorCounts.TryGetValue(doorId, out count);
            controllerDoorCounts[doorId] = count + 1;
        }

        private void RemoveControllerDoorReference(ulong doorId)
        {
            if (doorId == 0) return;

            int count;
            if (!controllerDoorCounts.TryGetValue(doorId, out count)) return;

            if (count <= 1)
            {
                controllerDoorCounts.Remove(doorId);
                return;
            }
        ...
        private static bool TryParseDelay(string value, out float time)
        {
            return float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out time)
                   || float.TryParse(value, NumberStyles.Float, CultureInfo.CurrentCulture, out time);
        }
        ...
        private bool IsValidDelay(float time)
        {
            return time >= configData.globalS.minimumDelay && time <= configData.globalS.maximumDelay;
        }

        private bool TryGetTargetDoor(BasePlayer player, out Door door, out string doorDisplayName)
        {
            door = GetLookingAtDoor(player);
            doorDisplayName = null;

            if (door == null || door.net == null) return false;
            return supportedDoors.TryGetValue(door.ShortPrefabName, out doorDisplayName);
        }

        private StoredData.DoorData GetOrCreateSingleDoorData(StoredData.PlayerData playerData, ulong doorId)
        {
            StoredData.DoorData doorData;
            if (!playerData.theDoorS.TryGetValue(doorId, out doorData))
            {
                doorData = CreateDefaultDoorData();
                playerData.theDoorS.Add(doorId, doorData);
            }

            return doorData;
        }

        private StoredData.DoorData GetOrCreateDoorTypeData(StoredData.PlayerData playerData, string shortPrefabName)
        {
            StoredData.DoorData doorData;
            if (!playerData.doorTypeS.TryGetValue(shortPrefabName, out doorData))
            {
                doorData = CreateDefaultDoorData();
                playerData.doorTypeS.Add(shortPrefabName, doorData);
            }

            return doorData;
        }

        private StoredData.DoorData CreateDefaultDoorData()
        {
            return new StoredData.DoorData
            {
                enabled = true,
                time = configData.globalS.defaultDelay
            };
        ...
        private void UpdateConfig()
        {
            ...
            foreach (var fallback in KnownDoorFallbacks)
            {
                ConfigData.DoorSettings doorSettings;
                if (!configData.doorS.TryGetValue(fallback.Key, out doorSettings))
                {
                    doorSettings = new ConfigData.DoorSettings
                    {
                        enabled = true,
                    ...
                    configData.doorS.Add(fallback.Key, doorSettings);
                }

                if (doorSettings.enabled && !supportedDoors.ContainsKey(fallback.Key))
                {
                    supportedDoors.Add(fallback.Key, doorSettings.displayName);
                }
            }
        ...
        private void CmdAutoDoor(BasePlayer player, string command, string[] args)
        {
            ...
            float time;
            if (TryParseDelay(args[0], out time))
            {
                if (!IsValidDelay(time))
                {
                ...
                if (!playerData.doorData.enabled)
                {
            ...
            switch (args[0].ToLowerInvariant())
            {
                ...
                case "all":
                {
                    if (args.Length > 1 && TryParseDelay(args[1], out time))
                    {
                        if (!IsValidDelay(time))
                        {
                            Print(player, Lang("AutoDoorDelayLimit", player.UserIDString, configData.globalS.minimumDelay, configData.globalS.maximumDelay));
                ...
                case "single":
                {
                    ...
                    if (!TryGetTargetDoor(player, out door, out doorDisplayName))
                    {
                        Print(player, Lang(GetLookingAtDoor(player) == null ? "DoorNotFound" : "DoorNotSupported", player.UserIDString));
                        return;
                    }

                    var doorData = GetOrCreateSingleDoorData(playerData, door.net.ID.Value);
                    var translatedDoorName = GetDeployableDisplayName(player, door.ShortPrefabName, doorDisplayName);

                    if (args.Length <= 1)
                    {
                        doorData.enabled = !doorData.enabled;
                        Print(player, Lang("AutoDoorSingle", player.UserIDString, translatedDoorName, doorData.enabled ? Lang("Enabled", player.UserIDString) : Lang("Disabled", player.UserIDString)));
                        return;
                    }

                    if (TryParseDelay(args[1], out time))
                    {
                        if (!IsValidDelay(time))
                        {
                            Print(player, Lang("AutoDoorDelayLimit", player.UserIDString, configData.globalS.minimumDelay, configData.globalS.maximumDelay));
                        ...
                        doorData.time = time;
                        Print(player, Lang("AutoDoorSingleDelay", player.UserIDString, translatedDoorName, time));
                        return;
                    }
                ...
                case "type":
                {
                    ...
                    var doorData = GetOrCreateDoorTypeData(playerData, door.ShortPrefabName);
                    var translatedDoorName = GetDeployableDisplayName(player, door.ShortPrefabName, doorDisplayName);

                    if (args.Length <= 1)
                    {
                        doorData.enabled = !doorData.enabled;
                        Print(player, Lang("AutoDoorType", player.UserIDString, translatedDoorName, doorData.enabled ? Lang("Enabled", player.UserIDString) : Lang("Disabled", player.UserIDString)));
                        return;
                    }
                    ...
                    if (TryParseDelay(args[1], out time))
                    {
                        ...
                        doorData.time = time;
                        Print(player, Lang("AutoDoorTypeDelay", player.UserIDString, translatedDoorName, time));
                        return;
                    }
